...
function parseDate(event) {
    ...
    const formattedDate = (0, _dateFns.format)(new Date(dateString), "yyyy-MM-dd");
    const startDate = new Date();
    const endDate = new Date(formattedDate);
    const days = (0, _dateFns.differenceInDays)(endDate, startDate);
    const years = (0, _dateFns.differenceInYears)(endDate, startDate);
    // const remainingDays = days % 365;
    const hours = (0, _dateFns.differenceInHours)(endDate, startDate) % 24;
    // форматирование ответа
    function formatYears(years) {
        if (years === 1) return `${years} год`;
        else if (years >= 2 && years <= 4) return `${years} года`;
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["gAoaA","6rimH"], "6rimH", "parcelRequired822")
